from src.agent.utils import read_markdown_file,extract_tools_from_module,update_tool_to_module,save_tool_to_module
...
class ToolAgent(BaseAgent):
    ...
    def generate_agent(self,state:AgentState):
        system_prompt=read_markdown_file('./src/agent/tool/generate_prompt.md')
        user_prompt='**Query:**\n`{query}`'
    ...
    def update_agent(self,state:AgentState):
        ...
        system_prompt=read_markdown_file('./src/agent/tool/update_prompt.md')
    ...
    def debug_agent(self,state:AgentState):
        if state.get('route')=='debug':
            ...
            tool_data=self.find_the_tool(input)
        ...
        while error and iteration<max_iteration:
            system_prompt=read_markdown_file('./src/agent/tool/debug_prompt.md')
            system_message=SystemMessage(system_prompt)
            ...
            debug_tool_data=self.llm.invoke([system_message,human_message],json=True)
            try:
                ast.parse(debug_tool_data.content.get('tool'))
                error=''
                update_tool_to_module(self.location,debug_tool_data.content)
                if self.verbose:
                    print(f'Fixed {tool_data.get("name")} and saved to {self.location} successfully.')
            except Exception as e:
                error=e
                iteration+=1
    ...
    def package_installer(self,state:AgentState):
        system_prompt=read_markdown_file('./src/agent/tool/package_installer_prompt.md')
    ...
    def router(self,state:AgentState):
        ...
        llm_router=LLMRouter(routes=routes,llm=self.llm,verbose=self.verbose)
